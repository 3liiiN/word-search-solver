 ...
 *
 * A program that solves word search puzzles of arbitrary dimensions
 * that are lowercase. Locates all words of length 6 or more that
 * appear horizontally, vertically, and diagonally, either forwards
 * or backwards (no wraparounds).
 *
 * Uses "words.txt" as dictionary
 * Uses "puzzle.txt" as puzzle
 *
 * Generates "answers.txt" to report found words alphabetically with
